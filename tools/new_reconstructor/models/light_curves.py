...
class LCPart(object):
    ...
    @staticmethod
    def postprocess(normed_x):
        raise NotImplementedError


class LinearPart(LCPart):
    @staticmethod
    def get_lc(normed_x):
        return (1 + normed_x)
...
class ExpPart(LCPart):
    @staticmethod
    def get_lc(normed_x):
        return pm.math.exp(normed_x)

    @staticmethod
    def postprocess(normed_x):
        return np.exp(normed_x)


class StitchedLC(LightCurve):
    E0 = E0_field()
    tau_left = tau_field()
    tau_right = tau_field()
    mu_k0 = DistributionField("normal", mu=0, sigma=1.0)
    ...
    def get_lc(self, delta_k, k0, const_storage):
        ...
        lpart = e0 * self.LEFT_PART.get_lc(delta_k_centered / tau1)
        rpart = e0 * self.RIGHT_PART.get_lc(-delta_k_centered / tau2)
    ...
    def _postprocess(self, delta_k, k0, ax, trace, pmt, actual_x):
        ...
        delta_k_centered = delta_k - mu_k0
        e0 = trace.get_estimation("E0")
        lpart = e0 * self.LEFT_PART.postprocess(delta_k_centered / tau1)
        rpart = e0 * self.RIGHT_PART.postprocess(-delta_k_centered / tau2)
...
class ExpolinearLC(StitchedLC):
    LEFT_PART = ExpPart
    RIGHT_PART = LinearPart

class LinexponentLC(StitchedLC):
    LEFT_PART = LinearPart
    RIGHT_PART = ExpPart

class ExpExpLC(StitchedLC):
    LEFT_PART = ExpPart
    RIGHT_PART = ExpPart

class LinLinLC(StitchedLC):
    LEFT_PART = LinearPart
    RIGHT_PART = LinearPart
...
def create_lc_alter():
    class LC_Alter(AlternatingNode):
        ...
        SEL__linexponent = LinexponentLC().generate_subform()
        SEL__expexp = ExpExpLC().generate_subform()
        SEL__linlin = LinLinLC().generate_subform()
