...
class LinearHardModel(ReconstructionModelWrapper):
    front_mul_sigma = FloatField(10.0)
    envelope_data = PassthroughField(EnvelopeSelector)
    discrete_time = BoolField(False)
    ...
    def get_pymc_model(self, observed):
        if self.envelope_data is None:
            ...
            envelope_func, envelope_uses_width = self.envelope_data
        with pm.Model() as model:
            ...
            if envelope_uses_width:
                front_mul = pm.HalfNormal("FrontMul", self.front_mul_sigma)
